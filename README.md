...
> **한 방에 받기** — 펌웨어(`bldc/`)는 이 레포의 **submodule**이다. 반드시 재귀 클론:
> ```bash
> git clone --recurse-submodules https://github.com/heedo7425/vesc_rc_failsafe.git
> # 이미 클론했다면: git submodule update --init --recursive
> ```
> 이 레포 하나면 펌웨어(빌드가능)+ROS 패치+문서 전부 들어있다.
...
```bash
cd bldc                       # submodule — 이미 feat/rc-failsafe-arbiter 에 고정돼 있음
make arm_sdk_install          # gcc-arm-none-eabi 7-2018-q2 자동 설치
make 60_mk6_hp                # -> build/60_mk6_hp/60_mk6_hp.bin  (MkVI는 make 60_mk6)
```
> submodule(`bldc/`)이 곧 fork [heedo7425/bldc@`feat/rc-failsafe-arbiter`] 이다.
> 순정 bldc에 직접 얹고 싶으면(submodule 없이): `release_6_05` 체크아웃 후
...
### 방법 A (권장) — 팀 스택의 vesc_driver 에 in-place 패치
자율주행 launch 가 띄우는 **바로 그 vesc_driver** 에 패치해야 통합 실행된다.
`git apply` 라 언제든 revert 되고, **커밋 안 하면 팀 공유 레포(원격)는 그대로** —
이 차의 로컬 working copy 만 바뀐다.
```bash
git clone https://github.com/heedo7425/vesc_rc_failsafe.git
cd vesc_rc_failsafe
./ros/patch/apply.sh  <vesc_driver 패키지 경로>
#   예: ./ros/patch/apply.sh ~/unicorn_ws/src/unicorn-racing-stack/sensor/vesc/vesc_driver
cd <ws> && colcon build --packages-select vesc_driver
# 되돌리기(순정 복원): ./ros/patch/revert.sh <같은 경로>
```
→ 이제 평소 자율주행 launch 로 그대로 실행하면 `vesc_driver` 가 `/rc/state` 발행 + 명령 게이트.

### 방법 B — 격리 테스트용 전용 ws (선택, 팀 스택 파일 무변경)
팀 스택을 파일째 안 건드리고 별도로 시험만 해보고 싶을 때. `vesc_msgs`/`vesc_driver`/
`transport_drivers` 를 복사해 패치된 전용 ws(`~/vesc_test`)를 만든다.
```bash
./ros/patch/setup_vesc_test.sh <팀_스택_src_경로>    # 예: ~/unicorn_ws/src
```
※ 실제 launch 는 이 전용 ws 를 안 쓰므로, 통합 실행은 방법 A 를 쓸 것.
...
| `ros/patch/apply.sh` / `revert.sh` | **팀 스택 vesc_driver 에 in-place 패치 / 원복 (권장)** |
| `ros/patch/setup_vesc_test.sh` | 격리 테스트용 전용 ws 자동 생성 (선택) |
| `ros/auto_test_pub.py` | 자동 모드 테스트 발행자 |
| `DESIGN.md` / `BRINGUP.md` | 설계 / 브링업 노트 · `firmware/*.lisp` LispBM PoC |

| `bldc/` | **submodule** → fork [heedo7425/bldc](https://github.com/heedo7425/bldc) `feat/rc-failsafe-arbiter` (빌드가능 펌웨어 전체) |
